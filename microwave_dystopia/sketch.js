...
function setup() {
  ...
  for (let i = 0; i < microwave.getRowCount(); i++){
    ...
    microwaveData.push(new MicrowaveData(date, time, sec, latitude, longitude, temp_item, item));
  }
...
function draw() {
  ...
  text(textTime, windowWidth/2 + 225, windowHeight/2 - 125);
  ...
  text(textLatitude, 150, 65);
  text(textLongitude, 250, 65);
...
class MicrowaveData{
  constructor(microwaveDate, time, sec, latitude, longitude, temp_item, item){
